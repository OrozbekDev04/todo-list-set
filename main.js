...
// isDone - Бул checked де  колдонулат.
// isImportant-Бул star да  колдунат.
...
const addItemTodoList = () => {
    ...
    const todoCheckbox = document.querySelectorAll('.todo_checked');
    ...
    const toDoDelete = document.querySelectorAll ('.todo_item-del')
    Array.from(toDoDelete).forEach(item => {
       item.addEventListener('click',()=>{
           console.log('click')
...
todoForm.addEventListener('submit',(event) => {
    ...
    if(tasks.some(el => el.text.toUpperCase() === event.target[0].value.toUpperCase())){
       alert('Сизде ката бар , сураныч текшерип корунуз . Катаныз - текст кайталанып жатат !')
